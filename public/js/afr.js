...
function ajax_handler($target, override) {
  ...
  if(id && options.handler) {
    ...
    //console.log("Submitting form id=" + id + " handler=" + options.handler + " success_handler=" + options.success_handler + " validator=" + validator + " reload_link=" + options.reload_link);
    //var url = "/home/app_data.php?oper=" + handler;
...
function datepicker_get_duration($dp, unit) {
  ...
  if($inp.val() && $out.val()) {
    ...
    var num_days = moment_out.diff(moment_in, unit, true);
    return num_days;
  }
  return 0;
}
...
function ready_script() {
  ...
  */
  ...
  if($('.bootstrap-toggle').length) {
    //console.log("Found " + $('.bootstrap-toggle').length + " bootstrap-toggle");
  ...
  if(apt.id) {
    ...
    options.events = function(start, end, timezone, callback) {
      ...
      $.ajax({
        ...
        success: function(events) {
          //var events = [];
          //console.log("Loaded events from " + checkin + " - " + checkout);
          //var eventObjs = $.parseJSON(response);
          $.each(events, function(i, event) {
...
$(document).on("click", ".fb-loader-popup", function() {
  ...
  console.log("fb-loader tpl=" + tpl + " ctrl=" + ctrl + " url=" + url);
...
$(document).on("click", ".subview-nav A", function() {
  var $link = $(this);
  var href = $link.attr('href');
  ...
  if(href && $target.length) {
    console.log("OK> Click " + href + " target=" + target + " len=" + $target.length);
    $($target).load(href + '?_pv=1'); // partial view
    return false;
  }
...
// old: new version of vmenu, load smarty template into div
...
function init_fileuploader($target, loadData, extra_options) {
  ...
  $target.fileupload({
    ...
    acceptFileTypes: uploader_options.acceptFileTypes, // /(\.|\/)(gif|jpe?g|png)$/i, // '/(\.|\/)(' + options.allowed_extensions.join('|') + ')$/i';
    ...
    autoUpload: false
     ...
     $.each(data.files, function (index, file) {
        //console.log(file);
        //console.log('Change: Added file: index=' + index + " name=" + file.name+ " size=" + file.size+ " type=" + file.type + " error=" + file['error']);
   ...
   $(document).on('click', "INPUT.check-all, INPUT[type='checkbox'][name='delete']", function() {
     console.log("delete click, wait");
     setTimeout(function(){console.log("run");jfu_del_update($target); }, 30);     
   });
   
   
   $target.bind('fileuploaddestroyed', function(e, data) { 
   ...
   $target.bind('fileuploadcompleted', function(e, data) {
   ...
   $target.bind('fileuploadstart', function(e, data) { 
     //console.log("fileuploadstart");
     //console.log(data);
   });      

   $target.bind('fileuploadsend', function(e, data) { 
   ...
   $target.bind('fileuploadadd', function(e, data) { 
   ...
   $target.bind('fileuploadfail', function(e, data) { // only called after successful add with no errors
     var file = data.files[0];
     var error = file.error;
     //console.log("fileuploadfail: Fail to add file: error =" + error);
   });      

   $target.bind('fileuploadfailed', function(e, data) { // only called after successful add with no errors
     //var file = data.files[0];
     //var error = file.error;
     //console.log(data);     
   ...
   $target.bind('fileuploadadded', function(e, data) { // only called after successful add with no errors
...
function jfu_del_update($target, del) {
  ...
  console.log("CB len= " + $cb.length + " Checked len=" + checked_count);
...
function jfu_ui_update($target) {
  ...
  if(download_count > 1) {
    ...
    if(view == 'list') {
      ...
      $file_cont = $target.parent().find('.file-table UL.file-container');
    }
