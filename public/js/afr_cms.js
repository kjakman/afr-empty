...
function layout_selectors_init(parent_id, css_selectors) {           
  for( var i = 0; i < css_selectors.length; i++ ) {
    ...
    if(css_selector.directional) {
      ...
      $cb.bind('change', function () {layout_direction_update(parent_id, $(this).data('css_selector'), true);});
...
function property_photo_handler(params, data) {
  //window.console && console.log("caller=", arguments.callee.caller.toString());
...
function new_org_handler(params, data, parent_id) {
  var copy_fields = ['salutation', 'first_name', 'last_name', 'address1', 'address2', 'zip', 'city', 'state', 'country', 'cell_phone', 'email_address'];
  //alert(dump(data));
...
function update_parent(data, parent_id) {
  ...
  var descriptor = data.obj_descriptor || data.org_name; // todo: make generic
...
function page_layout_handler(div_id, db_divs, data) {
  ...
  //$("#layout-panel .input").bind("change", function(){
