...
var g_cal_params = {
  ...
  "day_font_size":12, "grid_w":24, "grid_h":20,"title_w":150, 
  "bar_x":16,"bar_y":5, "status": 15, "show": "reservation"
};

g_cal_params.cell_h = g_cal_params.grid_h;
g_cal_params.cell_w = g_cal_params.grid_w;
...
function load_calendar_entries($params, $target) {  
  ...
  //console.log("Loading entries: ",params);
  ...
  $.getJSON(ajax_url, function(response) { // start loading data from server
    ...
    if(response.success) {
      ...
      if(params.target) $("#" + params.target).trigger('click'); // target reservation to hi-lite
    } else {
      var error = response.error || "Failed to load data";
...
function singleClick(e) {
  ...
  showMenu2($(this).data('obj_type'), $(this).data('obj_id'), event);
}
...
function menuContent(obj_data, cached) { // , indX, indY, cached) {
  ...
  var edit_button = " ";
  if(!cached && auth_edit) {
    edit_button = "<a href='#' class='edit_res' data-obj_type='" + obj_type + "' data-obj_id='" + obj_id + "'><i class='fa fa fa-pencil-square-o'></i></a>";
  }
  ...
  if(obj_data.org_name) menuTextAr.push(obj_data.org_name);
...
function cal_search_link_formatter(value, row, index) {
  ...
  ymd.w = 0;
  ymd.target = target;
  var nav_json = JSON.stringify(ymd);
  var link = row.link;
  var what = ' ' + ucfirst(obj_type) + ' ' + id;
  if(id && link) {
    public_link = link + id;
  ...
  text += '<a href="' + calendar_link + '" class="cal_nav start" data-dismiss="modal" data-value="' + start + '" data-target="' + target + '"><i class="fa fa-calendar">' + what + '</i></a>'
  return text;
}
...
function calendar_stats_guide(fields, dates, index) {
  ...
  for (var fld in fields) {
    ...
    box = '<div class="guide_title cell_w_font">' + title + '<br>' + subtitle + '</div><div class="guide-box"></div>';
    item = '<li class="cal_guide cal_day col_w">' + box + '</li>';
    items.push(item);
  }
  //console.log("Dates:",dates);
 
  var guide_title = dates.start_date + " - " + dates.end_date + " (" + dates.num_days + ' days)';
  var checked =  g_cal_params.tax_included  ? 'checked' : '';  
  console.log("tax incl=" + g_cal_params.tax_included);
  guide_title += "<div class='pull-right'>Incl. VAT? <input class='cal-stats-tax' type='checkbox' " + checked + "></div>";
...
$(document).on('click', '.cal_nav.start', function(e) {
  ...
  console.log("cal_nav start, timeout=" + g_timeout_interval);
...
$(document).on('click', '.cal_nav.period', function(e) {
  ...
  var cell_w = g_cal_params.grid_w;
  if(period > 6) {
    cell_w = 8;
  } else if(period > 4) {
    cell_w = 12;
  ...
  var font_size = 0.5 * cell_w
  g_cal_params.cell_w = cell_w;
...
function calendar_draw_entries(obj_data) {
  ...
  $.each(obj_data, function(i, obj) {
    ...
    //console.log("Draw " + obj_type + " " + obj_id + " @ " + apt_id + " in=" + start + " out=" + end);

    var start_offset = (start < cal_start) ? 0 : date2pos(ci, start) + res_x_offset;
    var end_offset = (end > cal_end) ? canvas_w : date2pos(ci, end);
    ...
    var style = 'height: ' + bar_h + 'px;width:' + width + 'px;top:' + top + 'px;left:' + left + 'px;';
    ...
    case 'reservation':
      ...
      break;
    case 'res_exception':
      var height = apt_id ? cell_h : canvas_h;
...
function calendar_get_stats(data) {
  ...
  var total, total_ex, rent, rent_ex, net, net_ex;
  
  console.log("range1 days=" + num_days + " range:", range1);
  
  /** todo: use fields */
  var blank_row =  {'days' : 0, 'occupancy': 0, 'rent': 0, 'rent_ex': 0, 'commission': 0};
  ...
  $.each(obj_data, function(i, data) {
    ...
    nd = data.num_days;
    total = data.total;
    total_ex = data.charge_tax && data.tax_included && tax_rate ? Math.round(total/tax_rate) : total;   
    ...
    rate = total/nd; // average rate per day (excl. fees, city tax)
    ...
    row['days'] += days; // count    
    row['rent'] += Math.round(rent); // count    
    row['rent_ex'] += Math.round(rent_ex); // count    
    rentals[0]['days'] += days; // sum total
    rentals[0]['rent'] += Math.round(rent); // sum total
    rentals[0]['rent_ex'] += Math.round(rent_ex); // sum total
    rentals[0]['commission'] += Math.round(commission); // sum total
  ...
  for (apt_id in rentals) {    
    ...
    ex_class = Math.round(rate) == Math.round(rate_ex) ? ' text-danger' : '';
    row['rate'] = '<span class="cal-tax-incl' + (g_cal_params.tax_included ? '' : ' collapse') + '">' + Math.round(rate) + '</span><span class="cal-tax-excl' + (g_cal_params.tax_included ? ' collapse' : '') + ex_class + '">' + Math.round(rate_ex) + '</span>';
    row['rent'] = '<span class="cal-tax-incl' + (g_cal_params.tax_included ? '' : ' collapse') + '">' + Math.round(rent) + '</span><span class="cal-tax-excl' + (g_cal_params.tax_included ? ' collapse' : '') + ex_class + '">' + Math.round(rent_ex) + '</span>';
    row['net']  = '<span class="cal-tax-incl' + (g_cal_params.tax_included ? '' : ' collapse') + '">' + Math.round(net)  + '</span><span class="cal-tax-excl' + (g_cal_params.tax_included ? ' collapse' : '') + ex_class + '">' + Math.round(net_ex) + '</span>';
...
function calendar_init() { 
  ...
  $("#cal-current").datepicker('setDate', start_date).on('changeDate', function(e) {
    ...
    console.log("Inline Change date to ", date,"=" + start_sql);
    load_cal({'start': start_sql});
  });

  // stats fields
  var fields = {'days':'Nights', 'occupancy':'Occ%', 'rate':"Rate", 'rent':'Rent', 'commission':'Comm.', 'net': "Net"};
...
function load_cal(nav, $target, timeout) {
  ...
  if(nav.period) g_cal_params.period = nav.period;
  ...
  var start = nav.start || data.start;

  if(nav.start) {
    var start_human = sql2human(nav.start);
