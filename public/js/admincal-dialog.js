...
function cal_booking_dialog(data, reset) {
  ...
  if(reset) {
    ...
    $("#cd-checkin").val(checkin);
    $("#cd-checkout").val(checkout); 
    $("#cd-reset").hide();
...
function edit_reservation($target) {
  ...
  var data = cached_data || {"obj_type": obj_type, "obj_id": obj_id, "apt_id": $target.data('apt_id'), "checkin": $target.data('in'), "checkout": $target.data('out')};
  
  if(g_new_dialog) { // add/edit booking
    console.log("edit_res: data=", data);
    cal_booking_dialog(data);
  } else {
    data = {"obj_type": obj_type, "res_id": obj_id, "property_type": g_cal_params.property_type};
    console.log("edit_res: data=", data);
    var  title = "Edit " + g_cal_params.obj_name + ' ' + obj_id;
    cal_dialog_old(title, data, options);
  }
  return false;
...
function cal_bind_date_handlers() {
  ...
  if(!rid) cal_update_period($("#booking_dates"));
...
function cal_update_period($dp) {
  if(!$dp) return;
  console.log("cal: update period: range id=" + $dp.attr('id'));
...
function cal_update_total() {
  ...
  var rate_type = $("#cd-rate_type").val().trim();
  var rate_base = $("#cd-rate_base").val().trim();
  var discount_str = $("#cd-discount").val().trim();
  var discount = 0;
  ...
  if(discount_str) {
    ...
    console.log("discont_str=" + discount_str + " base=" + rate_base + " discount_rate=" + discount_rate);
    total = count * discount_rate;
  }
  ...
  if(charge_city_tax && city_tax_rate) {    
    ...
    if(!isNaN(tax_percentage)) {
      city_tax = parseFloat(total * (tax_percentage/100));
      console.log("OK. Charge city tax: rate=" + city_tax_rate + " perc=" + tax_percentage + " tax=" + city_tax);
    }
  }
  ...
  /** grand total */
  var grand_total = 0;
  grand_total = total + city_tax + fees + service_fee;
...
function cal_update_rate_text(form_id) {
  ...
  $(".reservation-tax").html(formatFloat(city_tax));  
  ...
  if(city_tax > 0)  $("#dt_tax").show(); else $("#dt_tax").hide();   
...
function dialog_title(form_id) {
  ...
  var action = data.obj_id ? 'Edit ' + data.obj_type + ' ' + data.obj_id : 'New ' + data.obj_type;
  var checkin = $("#cd-checkin").val();
